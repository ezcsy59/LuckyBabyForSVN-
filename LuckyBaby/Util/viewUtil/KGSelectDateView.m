...
-(instancetype)initWithDateChanegSize:(NSInteger)changeSize{
    if (self = [super init]) {
        ...
        [leftBtn setImage:[UIImage imageNamed:@"left_icon.png"] forState:UIControlStateNormal];
        [leftBtn addTarget:self action:@selector(leftBtnClick) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:leftBtn];
        
        UIButton *rightBtn = [[UIButton alloc]init];
        rightBtn.frame = CGRectMake(265, 15, 20, 30);
        [rightBtn setImage:[UIImage imageNamed:@"right_icon.png"] forState:UIControlStateNormal];
