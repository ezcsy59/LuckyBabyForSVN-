...
-(void)rigthBtnClick{
    if (self.style == 0 || self.style == 2) {
        if (!(self.textField1.text.length > 0 && self.textField2.text.length > 0)) {
            self.tipView = [[KGTipView alloc]initWithTitle:nil context:@"填写信息不完整!" cancelButtonTitle:nil otherCancelButton:nil lockType:LockTypeGlobal delegate:nil userInfo:nil];
            [self.tipView show];
            return;
        }
