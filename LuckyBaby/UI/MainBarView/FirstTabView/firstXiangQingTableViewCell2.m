//
//  firstXiangQingTableViewCell2.m
//  LuckyBaby
//
...
@interface firstXiangQingTableViewCell2 ()
...
@implementation firstXiangQingTableViewCell2
...
-(void)setMainView{
    ...
    self.titleLabel.frame = CGRectMake(80, 10, 220, 20);
    ...
    self.xiangQingLabel = [[TQRichTextView alloc]initWithFrame:CGRectMake(0, 0, 220, 0)];
...
-(void)resetViewView:(NSDictionary *)dic{
    if (((NSString *)[DictionaryStringTool stringFromDictionary:dic forKey:@"firstType"]).length > 0) {
        self.titleLabel.text = [DictionaryStringTool stringFromDictionary:dic forKey:@"firstType"];
        self.titleLabel.text = [NSString stringWithFormat:@"活动:%@",self.titleLabel.text];
    }
    else{
    ...
    if ([photoArray isKindOfClass:[NSArray class]]) {
        self.photoImage.frame = CGRectMake(80, 10 + self.titleLabel.frame.origin.y + self.titleLabel.frame.size.height, 240, (((photoArray.count - 1)/3) + 1) *75);
        ...
        self.photoImage.frame = CGRectMake(80, 10 + self.titleLabel.frame.origin.y + self.titleLabel.frame.size.height, 240, 0);
    }
    ...
    if(self.xiangQingLabel.text.length > 0){
        ...
        self.xiangQingLabel.frame = CGRectMake(0, self.titleLabel2.frame.size.height + self.titleLabel2.frame.origin.y, 0, 0);
    }
    ...
    self.createdTimeLabel2.frame = CGRectMake(80 + 5, self.xiangQingLabel.frame.size.height + self.xiangQingLabel.frame.origin.y + 10, 200, 20);
    ...
    if ([commentArray isKindOfClass:[NSArray class]]) {
        ...
        for (int i = 0; i < commentArray.count; i++) {
            ...
            mainTextView.frame = CGRectMake(0, lastHeight, 220, mainTextView.drawheigth);
            lastHeight += mainTextView.drawheigth;
            [self.pingLunTextMainView addSubview:mainTextView];
        }
        self.pingLunTextMainView.frame = CGRectMake(80, self.createdTimeLabel2.frame.size.height + self.createdTimeLabel2.frame.origin.y + 10, 220, lastHeight);
